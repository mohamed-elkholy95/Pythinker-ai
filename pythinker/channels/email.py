...
CONFIG_FIELDS = {
    "label": "Email",
    "required_secrets": ["channels.email.imap_password", "channels.email.smtp_password"],
    "fields": [
        "enabled",
        "consent_granted",
        "imap_host",
...
class EmailChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        base_subject = self._last_subject_by_chat.get(to_addr, "pythinker reply")
        ...
        try:
            ...
            for imap_id in ids:
                ...
                if not self.is_allowed(sender):
                    logger.warning(
                        "Email from {} rejected: sender is not in allowFrom", sender,
                    )
                    self._remember_processed_uid(uid, dedupe, cycle_uids)
                    if mark_seen:
                        client.store(imap_id, "+FLAGS", "\\Seen")
    ...
    def _reply_subject(self, base_subject: str) -> str:
        subject = (base_subject or "").strip() or "pythinker reply"
