...
chmod 600 ~/.pythinker/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.pythinker/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.pythinker/config.json`) → **hidden** (masked by tmpfs)
...
- Keep authentication data in `~/.pythinker/whatsapp-auth` secure (mode 0700)
...
2. **Use a Dedicated User**
   ...
   sudo -u pythinker pythinker gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.pythinker
   chmod 600 ~/.pythinker/config.json
   chmod 700 ~/.pythinker/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.pythinker/logs/pythinker.log
   ```
...
- **Chat history is stored locally** - protect the `~/.pythinker` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.pythinker/logs/pythinker.log
   ```
