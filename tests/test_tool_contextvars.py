...
async def test_spawn_tool_keeps_task_local_context() -> None:
    ...
    class _Manager:
        async def spawn(self, *, task: str, label: str | None, origin_channel: str, origin_chat_id: str, session_key: str, **kwargs) -> str:
...
async def test_spawn_tool_basic_set_context_and_execute() -> None:
    ...
    class _Manager:
        async def spawn(self, *, task, label, origin_channel, origin_chat_id, session_key, **kwargs):
            seen.append((origin_channel, origin_chat_id, session_key))
            return f"ok: {task}"
