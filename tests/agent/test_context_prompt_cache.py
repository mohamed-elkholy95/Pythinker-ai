...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("pythinker") / "templates"
...
def test_identity_has_no_behavioral_instructions(tmp_path) -> None:
    ...
    assert "You are pythinker" not in identity
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("pythinker") / "templates" / "SOUL.md").read_text(encoding="utf-8")
    assert "## Execution Rules" in soul
    assert "single-step tasks" in soul
    assert "multi-step tasks" in soul

def test_default_tool_template_explains_browser_tool_choice(tmp_path) -> None:
    """Default bootstrap guidance should distinguish web_fetch from browser."""
    from pythinker.utils.helpers import sync_workspace_templates
    ...
    assert "Use `web_fetch` for static pages" in prompt
    assert "When the `browser` tool is registered" in prompt
    assert "headless Chromium" in prompt
    assert "not the user's personal GUI browser" in prompt
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by pythinker" not in prompt
