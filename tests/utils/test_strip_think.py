...
class TestStripThinkMalformedLeaks:
    ...
    (e.g. `<thinkéclairage…`). The earlier regexes required '>' and
    let these through."""

    def test_malformed_think_no_gt_non_ascii(self):
        assert strip_think("<thinkéclairage actuellement lié au style 'éclairage'") == (
            "éclairage actuellement lié au style 'éclairage'"
        )
    ...
    def test_malformed_thought_no_gt(self):
        assert strip_think("<thoughtéclairage") == "éclairage"
    ...
    def test_channel_marker_at_start_stripped(self):
        ...
        assert strip_think("<channel|>fountain schedule: opens at 09:00") == ("fountain schedule: opens at 09:00")
