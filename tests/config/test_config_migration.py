...
def test_save_config_preserves_context_window_tokens_but_not_memory_window(tmp_path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        json.dumps(
            {
...
def test_onboard_does_not_crash_with_legacy_memory_window(tmp_path, monkeypatch) -> None:
    ...
    # Migration tests only exercise the load/refresh path; skip the wizard.
    monkeypatch.setattr(
        "pythinker.cli.onboard.run_onboard",
        lambda *args, **kw: OnboardResult(config=Config(), should_save=False),
    )
...
def test_onboard_refresh_backfills_missing_channel_fields(tmp_path, monkeypatch) -> None:
    ...
    # Migration tests only exercise the channel-defaults injection path; skip the wizard.
    monkeypatch.setattr(
        "pythinker.cli.onboard.run_onboard",
        lambda *args, **kw: OnboardResult(config=Config(), should_save=False),
    )
