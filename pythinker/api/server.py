"""OpenAI-compatible HTTP API server for a fixed pythinker session.
...
# Typed Application keys (replaces string keys; silences NotAppKeyWarning
# and gives static checkers / future maintainers the value type at lookup).
# See aiohttp web_advanced "Data Sharing" — AppKey is the recommended
# pattern since v3.9 for storing app-scoped state.
_AGENT_LOOP_KEY: web.AppKey[Any] = web.AppKey("agent_loop", Any)
_MODEL_NAME_KEY: web.AppKey[str] = web.AppKey("model_name", str)
_REQUEST_TIMEOUT_KEY: web.AppKey[float] = web.AppKey("request_timeout", float)
_SESSION_LOCKS_KEY: web.AppKey[dict[str, asyncio.Lock]] = web.AppKey(
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    # -- streaming path --
    if stream:
        # Pre-authorize before sending any headers, AND capture the resulting
        # RequestContext. We pass that same ctx back into process_direct so
        # one logical request produces one trace_id and one ingress
        # policy_decision event.
        try:
            preauth_ctx = agent_loop.preauthorize_direct(
            ...
            logger.warning(
                "Ingress policy denied streaming API request for session_hash={}: {}",
                hash_identity(session_key), pe,
            )
            return _error_json(
                403, str(pe) or "Forbidden",
                ...
                async with session_lock:
                    await asyncio.wait_for(
                        agent_loop.process_direct(
                            ...
                            request_context=preauth_ctx,  # reuse, do NOT re-normalize
                        ),
                        timeout=timeout_s,
                    )
            except Exception:
    ...
    # Pre-authorize once and reuse the same RequestContext across both
    # the primary call and the empty-response retry. Without a shared
    # context, process_direct() mints a fresh trace_id and a fresh
    # BudgetCounters on the retry — splitting one HTTP request into two
    # independent governed contexts and bypassing per-request budget
    # continuity. PermissionError on pre-auth → 403 (matches streaming).
    try:
        preauth_ctx = agent_loop.preauthorize_direct(
            session_key=session_key,
            channel="api",
        ...
        logger.warning(
            "Ingress policy denied API request for session_hash={}: {}",
            hash_identity(session_key), pe,
        )
        return _error_json(
            403, str(pe) or "Forbidden",
    ...
    try:
        async with session_lock:
            try:
                ...
                logger.exception(
                    "Error processing request for session_hash={}",
                    hash_identity(session_key),
                )
...
def create_app(
    agent_loop, model_name: str = "pythinker", request_timeout: float = 120.0
) -> web.Application:
