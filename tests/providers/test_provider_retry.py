...
from pythinker.providers.base import GenerationSettings, LLMProvider, LLMResponse
...
async def test_chat_with_retry_retries_transient_error_then_succeeds(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pythinker.providers.base.asyncio.sleep", _fake_sleep)
...
async def test_non_transient_error_with_images_retries_without_images() -> None:
    ...
    provider = ScriptedProvider([
        LLMResponse(content="API call parameters are invalid, please check the docs", finish_reason="error"),
...
async def test_chat_with_retry_retries_zhipu_1302_rate_limit(monkeypatch) -> None:
    """ZhiPu returns code 1302 with a rate-limit message instead of HTTP 429."""
    provider = ScriptedProvider([
        LLMResponse(
            content="Error: {'code': '1302', 'message': 'Your account has hit the rate limit, please reduce request frequency'}",
            finish_reason="error",
        ),
        LLMResponse(content="ok"),
    ])
