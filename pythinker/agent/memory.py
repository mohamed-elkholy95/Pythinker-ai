...
class Dream:
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        from pythinker.agent.tools.filesystem import EditFileTool, ReadFileTool, WriteFileTool
    ...
    async def run(
        ...
        *,
        ...
        egress: "ToolEgressGateway | None" = None,
    ) -> bool:
        """Process unprocessed history entries. Returns True if work was done.

        When ``request_context`` and ``egress`` are both provided, Phase 2's
        AgentRunner routes every tool call through the gateway under the given
        context (the cron callback passes a ``system_dream``-bound context so
        PolicyService applies the named exemption). Both default to None for
        legacy callers — without them the runner falls back to direct
        ``tools.execute`` dispatch (no governance, no telemetry). Pass them
        together: the AgentRunSpec XOR guard rejects setting only one.
        """
        from pythinker.agent.skills import BUILTIN_SKILLS_DIR
        ...
        try:
            result = await self._runner.run(AgentRunSpec(
                ...
                # Egress and request_context are wired together (XOR guard);
                # gate egress on the context to keep legacy callers untouched.
                egress=egress if request_context is not None else None,
            ))
            logger.debug(
